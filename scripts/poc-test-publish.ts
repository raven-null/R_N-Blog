...
const fc = (r.data?.data || []).find((a: any) => a.id === cardId)
check("14b) filter=card&withContent=1 附带正文", r.status === 200 && !!fc && String(fc.content || "").includes("卡片"), JSON.stringify({ found: !!fc, len: fc ? String(fc.content || "").length : -1 }))
