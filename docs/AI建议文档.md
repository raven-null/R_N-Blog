...
| 风险 | 对策 |
|------|------|
| 恶意刷任务刷成本 | 生成文章需输入**正确的后台密钥**；问答用 AI 助手且默认存草稿；单任务 token 上限 |
...
| P1 | 评论触发 `@管理员`（需密钥）+ 受理 / 完成评论 | 核心场景 B | 中 |
| P1 | 异步执行（后台函数 + 队列）+ 任务状态 | 长文与体验 | 中 |
| P1 | 评论 `@ai` 文章问答（AI 助手回答） | 互动场景 | 中 |
...
- **密钥保护**：生成文章需后台密钥，避免公共接口被刷成本；问答无需密钥但依赖「AI 助手」配置
