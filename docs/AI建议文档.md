...
- 品牌标签（hero-tag）：保留，可放大作为视觉主角（字号 16-20px、加宽字距）
- 副标题：保留并放大（作为次主角），支持左对齐 + 左侧装饰线排版
- **CTA 主按钮**："开始阅读"（平滑滚动到文章区），替换/补充 Scroll 指示
- 删除：`hero-title` 大标题、`hero-stats` 三列统计

#### 4.4 统计信息拆分（移出 Hero）

- **文章数、标签数**（拆分方案三选一）：
  - 方案 ①：内容区顶部（文章瀑布流上方）加一条细统计栏，如 `文章 N 篇 · 标签 M 个`
  - 方案 ②：并入导航栏（右侧显示小徽标）
  - 方案 ③：放入页脚
- **字数统计**：直接删除，不再展示
