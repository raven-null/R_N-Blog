...
## 十、Hero 区重新设计方案（解决首页视觉）

> 目标：让首屏更有设计感、层次清晰、不杂乱。

### 1. 现状问题

- 装饰元素过多：光球×3、脉冲环×3、浮动线×3、粒子爆发×6、角标×4、装饰线、网格闪光、渐变背景、粒子 Canvas 同时叠加
- 元素多但层次乱，视觉重心不明确，观感"满而杂"
...
| 方案 | 描述 | 适合 |
|------|------|------|
| A 极简居中 | 大面积渐变背景 + 细腻粒子 + 居中大标题，大量留白 | 品牌感、克制 |
| B 沉浸式背景 | 全屏动态渐变/大图，内容浮于其上，毛玻璃文字 | 视觉冲击 |
| C 左文右图 | 左侧品牌信息 + CTA，右侧装饰视觉（代码/光效/图形） | 信息型首页 |

**推荐组合：B/A 融合** —— 全屏柔和动态渐变 + 适度粒子，内容居中强化，删除重复装饰。
...
- 目标：首屏元素收敛到「背景动效 + 内容」两层

#### 3.2 背景升级

- 大面积径向渐变（主题色系），缓慢流动（`background-position` 或 Anime.js 驱动 CSS 变量）
- 可选叠加轻微网格/噪点（`opacity 0.02~0.05`），增加质感
- 移动端降级为静态渐变（省性能）

#### 3.3 内容层级强化

- 品牌标签（hero-tag）：保留，收窄间距
- 主标题：字号加大、渐变文字（已有）、逐字动画（已实现），可增加轻微"光晕呼吸"
- 副标题：左对齐 + 左侧装饰线，提升排版质感
- **新增 CTA 主按钮**："开始阅读"（平滑滚动到文章区），替换/补充 Scroll 指示
- 统计数字：三列保留，改为卡片/分隔线分隔，弱化装饰

#### 3.4 动效（Anime.js 已就绪）

- 背景渐变缓慢流动（`duration: 8000, loop: true`）
- 鼠标跟随视差：标题/装饰层随鼠标微移（`translateX/Y` 基于鼠标位置）
...
- 标题 `clamp()` 自适应（已有），CTA 全宽更易点击

### 4. 建议实施步骤

1. 先做"减法"：删除重复装饰元素与对应 CSS/JS，评估首屏
2. 升级背景为动态渐变（Anime.js）
3. 新增 CTA 主按钮 + 副标题排版优化
4. 加鼠标视差与滚动联动
5. 移动端降级适配，回归测试
...
| P0 | 删除冗余装饰（减法） | 首屏立即清爽 |
| P0 | 背景动态渐变 | 质感提升 |
| P1 | CTA 主按钮 + 副标题排版 | 行动引导 |
| P1 | 鼠标视差 / 滚动联动 | 沉浸感 |
| P2 | 移动端降级适配 | 性能与观感 |

---

**状态：** AI 助手部分已实施（v2.2.0）；Anime.js 动画方案已实施（v2.4.0）；首页改造方案已实施（v2.4.2，P0/P1 项）；标签区改为导航栏"全部标签"面板（v2.4.5），Hero 重设计待评审
**作者：** 渡鸦NULL
