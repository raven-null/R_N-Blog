...
const IMAGE_STORE = "avatar-images"
...
/**
 * /api/avatar-image  (Netlify Functions v2)
 *
 * 头像独立存储，与图库（blog-images）、文章图片（article-images）完全隔离。
 *
 * POST { data: "<base64>", mime: "image/png" }  上传头像，返回 { url }
 * GET  ?key=xxx                                 读取头像
 * DELETE ?key=xxx                               删除头像（需 X-Admin-Key）
 */
export default async (req: Request) => {
  ...
  if (req.method === "POST") {
    ...
    return json(200, {
      status: "success",
      url: `/api/avatar-image?key=${encodeURIComponent(finalKey)}`,
      key: finalKey,
      converted: !isSvg,
    }, req)
  }
...
export const config = { path: "/api/avatar-image" }
